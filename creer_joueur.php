...
<main class="ml-0 md:ml-48 flex flex-col items-center justify-center p-6 min-h-screen">
  ...
  <?php if (!empty($_SESSION['toast'])): ?>
    <div class="mb-4 px-4 py-2 bg-primary rounded shadow">
  ...
  <form method="post" enctype="multipart/form-data"
        class="max-w-4xl w-full flex flex-col md:flex-row gap-8">
    ...
    <section class="md:flex-1 bg-black/70 border-2 border-white/60 rounded-xl p-6 shadow-lg space-y-4">
      ...
      <div class="flex gap-4">
        <div class="flex-1">
          ...
          <input name="nom_joueur" required value="<?= htmlspecialchars($_POST['nom_joueur'] ?? '') ?>"
                 class="w-full p-2 rounded text-black">
        </div>
        <div class="flex-1">
          <label class="block text-sm mb-1">Prénom *</label>
          <input name="prenom_joueur" required value="<?= htmlspecialchars($_POST['prenom_joueur'] ?? '') ?>"
                 class="w-full p-2 rounded text-black">
        </div>
      </div>
      ...
      <div class="flex gap-4">
        ...
        <div class="flex-1">
          ...
          <input type="number" name="poids_kg" min="30" max="250"
      ...
      <select name="poste" required class="w-full p-2 rounded text-black">
        ...
        <?php
          $postes = ["Pilier gauche","Talonneur","Pilier droit","Deuxième ligne",
    ...
    <aside class="md:w-80 flex flex-col items-stretch gap-4">
      <div class="bg-black/70 border-2 border-white/60 rounded-xl p-6 text-center shadow-lg space-y-4">
        ...
        <img id="preview" src="images/default_avatar.jpg" alt="Aperçu"
      ...
      <a href="joueurs.php"
         class="text-center bg-danger hover:bg-red-800 py-2 rounded shadow">Retour</a>
    </aside>
  </form>
</main>
<script>
